...
void publishUptime(){
  ...
  mqttClient.publish(("smarthome/things/" + dev_uid + "/uptime/state").c_str(), 0, true, String(newuptime.getTotalSeconds()/60).c_str());
}

void publishAvailability() {
  mqttClient.publish(String("smarthome/things/" + dev_uid + "/status").c_str(), 0, true, "ONLINE");
}

void publishMode(){
  mqttClient.publish(String("smarthome/items/" + dev_uid + "_hvac_mode/state").c_str(), 0, true, mode.c_str());
}

void publishTemp(){
  mqttClient.publish(String("smarthome/items/" + dev_uid + "_temperature/state").c_str(), 0, true, String(temperature).c_str());
}

void publishFanSpeed(){
  mqttClient.publish(String("smarthome/items/" + dev_uid + "_fan_mode/state").c_str(), 0, true, String(fan_speed).c_str());
}

void publishSwing() {
  mqttClient.publish(("smarthome/items/" + dev_uid + "_hvac_swing_mode/state").c_str(), 0, true, swing ? "on" : "off");
}

void publishBoost() {
  mqttClient.publish(("smarthome/items/" + dev_uid + "_hvac_preset_mode/state").c_str(), 0, true, boost ? "boost" : "none");

}

void publishMQTTConfig() {
  String configUT = "{\"icon\": \"mdi:timer-outline\",\"availability\":{\"topic\":\"smarthome/things/" + dev_uid + "/status\",\"payload_available\":\"ONLINE\",\"payload_not_available\":\"OFFLINE\"},\"name\": \"" + entity_name + " uptime\",\"unit_of_measurement\": \"m\",\"device\":{\"name\":\"" + dev_name + "\",\"model\":\"" + dev_model + "\",\"manufacturer\":\"Dmitrii Nechaevskii\",\"sw_version\":\"" + sw_ver + "\",\"identifiers\":\"" + dev_uid + "\",\"connections\":[[\"mac\",\"" + FSReadMAC() + "\", \"ip\",\"" + WiFi.localIP().toString() + "\"]]},\"unique_id\": \"" + dev_uid + "_uptime\",\"device_class\": \"duration\",\"state_topic\": \"~state\",\"state_class\": \"total_increasing\",\"entity_category\": \"diagnostic\",\"~\":\"smarthome/things/" + dev_uid + "/uptime/\"}";

  String configReboot = "{\"availability\":{\"topic\":\"smarthome/things/" + dev_uid + "/status\",\"payload_available\":\"ONLINE\",\"payload_not_available\":\"OFFLINE\"},\"name\": \"" + entity_name + " reboot\",\"device\":{\"name\":\"" + dev_name + "\",\"model\":\"" + dev_model + "\",\"manufacturer\":\"Dmitrii Nechaevskii\",\"sw_version\":\"" + sw_ver + "\",\"identifiers\":\"" + dev_uid + "\",\"connections\":[[\"mac\",\"" + FSReadMAC() + "\", \"ip\",\"" + WiFi.localIP().toString() + "\"]]},\"unique_id\": \"" + dev_uid + "_reboot\",\"command_topic\": \"~command\",\"payload_press\": \"restart\",\"entity_category\": \"diagnostic\",\"~\":\"smarthome/things/" + dev_uid + "/reboot/\"}";
  ...
  config += "\"availability\": {";
  config += "\"topic\": \"smarthome/things/" + dev_uid + "/status\",";
  config += "\"payload_available\": \"ONLINE\",";
  config += "\"payload_not_available\": \"OFFLINE\",";
  config += "\"value_template\": \"{{ value_json.status }}\"";
  config += "},";
  config += "\"current_temperature_topic\": \"smarthome/items/" + dev_uid + "_inside_temperature/state\",";
  config += "\"fan_mode_command_topic\": \"smarthome/items/" + dev_uid + "_fan_mode/set\",";
  config += "\"fan_mode_state_topic\": \"smarthome/items/" + dev_uid + "_fan_mode/state\",";
  config += "\"fan_modes\": [";
  config += "\"auto\",";
  config += "\"low\",";
  config += "\"medium\",";
  config += "\"high\"";
  config += "],";
  config += "\"mode_command_topic\": \"smarthome/items/" + dev_uid + "_hvac_mode/set\",";
  config += "\"mode_state_topic\": \"smarthome/items/" + dev_uid + "_hvac_mode/state\",";
  config += "\"modes\": [";
  config += "\"auto\",";
  config += "\"cool\",";
  config += "\"heat\",";
  ...
  config += "\"preset_mode_command_topic\": \"smarthome/items/" + dev_uid + "_hvac_preset_mode/set\",";
  config += "\"preset_mode_state_topic\": \"smarthome/items/" + dev_uid + "_hvac_preset_mode/state\",";
  config += "\"preset_modes\": [";
  config += "\"boost\"";
  config += "],";

  config += "\"swing_mode_command_topic\": \"smarthome/items/" + dev_uid + "_hvac_swing_mode/set\",";
  config += "\"swing_mode_state_topic\": \"smarthome/items/" + dev_uid + "_hvac_swing_mode/state\",";
  config += "\"swing_modes\": [";
  config += "\"on\",";
  config += "\"off\"";
  config += "],";
  ...
  config += "\"precision\": 1,";
  config += "\"retain\": true,";
  config += "\"temperature_command_topic\": \"smarthome/items/" + dev_uid + "_temperature/set\",";
  config += "\"temperature_state_topic\": \"smarthome/items/" + dev_uid + "_temperature/state\",";
  config += "\"temp_step\": 1,";
  config += "\"device\": {";
...
void onMqttConnect(bool sessionPresent) {
  ...
  uint16_t packetIdSub0 = mqttClient.subscribe(String("smarthome/items/" + dev_uid + "_pause").c_str(), 0); // QoS 0
  Serial.print("Subscribing at QoS 0, packetId: ");
  Serial.println(packetIdSub0);
  uint16_t packetIdSub1 = mqttClient.subscribe(String("smarthome/items/" + dev_uid + "_hvac_mode/set").c_str(), 0); // QoS 0
  Serial.print("Subscribing at QoS 0, packetId: ");
  Serial.println(packetIdSub1);
  uint16_t packetIdSub2 = mqttClient.subscribe(String("smarthome/items/" + dev_uid + "_temperature/set").c_str(), 0);
  Serial.print("Subscribing at QoS 0, packetId: ");
  Serial.println(packetIdSub2);
  uint16_t packetIdSub3 = mqttClient.subscribe(String("smarthome/items/" + dev_uid + "_fan_mode/set").c_str(), 0);
  Serial.print("Subscribing at QoS 0, packetId: ");
  Serial.println(packetIdSub3);
  uint16_t packetIdSub4 = mqttClient.subscribe(String("smarthome/items/" + dev_uid + "_hvac_preset_mode/set").c_str(), 0);
  Serial.print("Subscribing at QoS 0, packetId: ");
  Serial.println(packetIdSub4);
  uint16_t packetIdSub5 = mqttClient.subscribe(String("smarthome/items/" + dev_uid + "_hvac_swing_mode/set").c_str(), 0);
  Serial.print("Subscribing at QoS 0, packetId: ");
  Serial.println(packetIdSub5);
  uint16_t packetIdSub6 = mqttClient.subscribe(String("smarthome/items/" + dev_uid + "/reboot/command").c_str(), 0);
  Serial.print("Subscribing at QoS 0, packetId: ");
  Serial.println(packetIdSub6);
  
  publishMQTTConfig();
  
  mqttClient.publish(String("smarthome/things/" + dev_uid + "/status").c_str(), 0, false, "ONLINE");
}
...
void onMqttMessage(char *in_topic, char *payload, AsyncMqttClientMessageProperties properties, size_t len, size_t index, size_t total) {
  ...
  if (String(in_topic) == "smarthome/items/" + dev_uid + "_hvac_mode/set") {
    String messageMQTT;
  ...
  if (String(in_topic) == "smarthome/items/" + dev_uid + "_fan_mode/set") {
    String messageMQTT;
    for (size_t i = 0; i < len; i++) {
      messageMQTT += (char)payload[i];
    }
  ...
  if (String(in_topic) == "smarthome/items/" + dev_uid + "_temperature/set") {
    String messageMQTT;
    for (size_t i = 0; i < len; i++) {
      messageMQTT += (char)payload[i];
    }
  ...
  if (String(in_topic) == "smarthome/items/" + dev_uid + "_hvac_preset_mode/set") {
    String messageMQTT;
    for (size_t i = 0; i < len; i++) {
      messageMQTT += (char)payload[i];
    }
  ...
  if (String(in_topic) == "smarthome/items/" + dev_uid + "_hvac_swing_mode/set") {
    String messageMQTT;
    for (size_t i = 0; i < len; i++) {
      messageMQTT += (char)payload[i];
    }
  ...
  if (String(in_topic) == "smarthome/items/" + dev_uid + "/reboot/command") {
    String messageMQTT;
    for (size_t i = 0; i < len; i++) {
      messageMQTT += (char)payload[i];
    }
...
void startServer() {
  ...
  server.on("/setDeviceCred", HTTP_POST, [](AsyncWebServerRequest *request) {
    ...
    if (request->hasParam("deviceName", true) && request->hasParam("entName", true)) {
      FSWriteDeviceName(request->getParam("deviceName", true)->value());
      FSWriteEntityName(request->getParam("entName", true)->value());
      flag_time = millis();
      restart_flag = true;
      request->send(403, "text/plain", "Name & Entity config changed, reboot in 10sec...");
    } else {
      request->send(400, "text/plain", "Bad Request: Name or Entity not provided");
    }
  });
  ...
  server.on("/newwifi", HTTP_POST, [](AsyncWebServerRequest *request) {
    ...
    if (request->hasParam("wifiname", true) && request->hasParam("wifipwd", true)) {
      ...
      flag_time = millis();
      restart_flag = true;
      request->send(403, "text/plain", "Wifi config changed, reboot in 10sec...");
    } else {
  ...
  server.on("/newmqtt", HTTP_POST, [](AsyncWebServerRequest *request) {
    ...
    if (request->hasParam("mqttip", true) && request->hasParam("mqttport", true)) {
      ...
      if (config_present) {
        flag_time = millis();
        restart_flag = true;
        request->send(200, "text/plain", "MQTT updated, reboot in 10sec...");
      } else {
  ...
  server.on("/hardreset", HTTP_POST, [](AsyncWebServerRequest *request) {
    ...
    flag_time = millis();
    restart_flag = true;
    request->send(403, "text/plain", "Full config reset, memory cleaned, reboot in 10sec...");
  });
  ...
  server.on("/reboot", HTTP_POST, [](AsyncWebServerRequest *request) {
    ...
    flag_time = millis();
    restart_flag = true;
    request->send(403, "text/plain", "Reboot in 10sec...");
  });

  server.on("/acon", HTTP_POST, [](AsyncWebServerRequest *request) {
    if (!isAuthenticated(request)) {
      request->send(403, "text/plain", "Not Authorized");
      return;
    }
  ...
  server.on("/acoff", HTTP_POST, [](AsyncWebServerRequest *request) {
    if (!isAuthenticated(request)) {
      request->send(403, "text/plain", "Not Authorized");
      return;
    }
  ...
  server.on("/update", HTTP_POST, [](AsyncWebServerRequest *request) {
    ...
    request->send(200, "text/plain", "Update started, wait for completion...");
...
void setup() {
  ...
  config_topic = "homeassistant/climate/" + dev_uid + "/" + dev_uid + "/config";
  ...
  // SUPERIMPORTANT!!!
  static String willTopic = "smarthome/things/" + dev_uid + "/status";
